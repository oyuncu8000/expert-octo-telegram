...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .live-pill {
    ...
    border: 1.5px solid rgba(229,9,20,.65);
    color: #fff;
  ...
  @keyframes livePulse {
    0%,100% { opacity:1; box-shadow: 0 0 0 0 rgba(229,9,20,.8); }
    50%      { opacity:.5; box-shadow: 0 0 0 5px rgba(229,9,20,0); }
  }

  /* Age Rating — Netflix/Prime badge */
  .age-badge {
    display: flex;
    align-items: center;
    gap: 6px;
  ...
  @keyframes liveGlow {
    0%,100% { box-shadow: none; opacity: .9; }
    50%      { box-shadow: 0 0 10px rgba(229,9,20,.7); opacity: 1; }
  }
  ...
  #volSlider::-webkit-slider-runnable-track {
    background: linear-gradient(90deg, #fff var(--v,80%), rgba(255,255,255,.25) var(--v,80%));
  ...
  #statusScreen {
    ...
    background: #000; transition: opacity .5s;
  }
  ...
  @media (max-width: 480px) {
    #topBar    { padding: 16px; }
    #bottomBar { padding: 0 16px 20px; }
    .ctrl-btn  { width: 36px; height: 36px; }
    #playBtn   { font-size: 1.2rem; }
    .pill-btn  { padding: 5px 10px; font-size: .64rem; }
    #volWrap   { width: 80px; }
    .age-num   { font-size: .8rem; }
  }
</style>
</head>
<body>
...
<div id="overlay">
  <div id="topBar">
    ...
    <div class="age-badge">
      <div class="age-num">13+</div>
      <div class="age-label">Yaş<br>ve Üzeri</div>
    </div>
  </div>
    ...
    <div id="ctrlRow">
      ...
      <div id="volWrap">
        <input type="range" id="volSlider" min="0" max="100" value="80"
          oninput="onVolume(this.value)" style="--v:80%">
      </div>
...
<script>
const STREAM = 'https://app.viloud.tv/hls/channel/09368cef97b0a015a45970bea736e8e0.m3u8';
...
function showOverlay() {
  ...
  idleTimer = setTimeout(() => overlay.classList.remove('visible'), 3500);
}
